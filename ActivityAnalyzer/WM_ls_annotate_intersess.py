"""
    ...
    ADAPTED FROM WM_ls_annotate_bywell. Shows images between sessions instead of within a session
"""
...
def extractROI(rois,session_idx,buf, r = 0):
...
def getWormScore(crops,buf,r, session_idx):
    ...
    while(True):
        ...
        if key == 49:
            ...
            print('Cannot go back past first session !!')
...
for (psession,session_idx) in zip(psessions,range(N_sessions)):
    ...
    current_images = list()
    current_images.append(cv2.imread(fimg[0],cv2.IMREAD_COLOR))
...
while well_idx < N_wells:
    ...
    # If this well is already analyzed, skip it
    ...
    while True:
        ...
        print("Session ", session_idx, " of " , N_sessions , " Well index ", well_idx, "... ", end='', flush=True)
        #sys.stdout.write("Session " + str(session_idx) + " of " + str(N_sessions) + " Well index " + str(well_idx) + "... ")
        ...
        # Enforce ROI is same size throughout - DISABLED. TO RE-ENABLE SUPPLY AS ARG TO extractROI
        # r = roi[2,0]


        # Extract ROI for this session and next session
        x0,y0,x1,y1,xc,yc, r = extractROI(rois,session_idx,buf)
        x0a,y0a,x1a,y1a,xca,yca, r = extractROI(rois,session_idx+1,buf)


        # Add both crops

        this_frame = images[session_idx][0]
        next_frame = images[session_idx+1][0]

        crops = list()
        crops.append(np.copy(this_frame[y0:y1,x0:x1]))
        crops.append(np.copy(next_frame[y0a:y1a,x0a:x1a]))
        ...
        if decrement_session_flag:
            ...
            sessions_list = sessions_list[0:-2]
            print('',flush=True)
            continue
