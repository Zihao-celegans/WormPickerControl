...
public:
	...
	* the string.
	*/
	std::string readStringUntil(const std::string delim = "\r");

	/*
		Read all lines of output OR wait for any line to be available
	*/
	void readStringAllLines();

	std::string readRecent();
	//void waitForResponse(boost::posix_time::time_duration t = boost::posix_time::hours(999));
...
private:
	...
	*/
	...
	int timerIsExpired;							/// Whether the timer has finished
